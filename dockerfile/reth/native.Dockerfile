...
RUN if [ ! -z "$BUILD_TARGET" ]; then \
      if [ ! -z "$BUILD_DIR" ]; then cd "${BUILD_DIR}"; \
      fi; \
      if [ ! -f "Cargo.toml" ]; then exit 0; \
      fi; \
      cargo fetch; \
    fi
...
# Final image: Alpine (to match your side-by-side pattern)
FROM alpine:latest
...
# Install chain binaries and libs
